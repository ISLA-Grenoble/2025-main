...
std = est.steps[1][1].singular_values_ / np.sqrt(len(df))
...
for j in range(len(Q)):
    ax.arrow(x=0.0, y=0.0, dx=std[j]*Q[j][0], dy=std[j]*Q[j][1],
             ec='r', fc='r', lw=0.8, head_width = 0.1)
    ax.text(x=std[j]*Q[j][0], y=std[j]*Q[j][1], s=df.columns[j], c='r')
ax.set_xlim(-6, 7)
ax.set_ylim(-6, 6)
