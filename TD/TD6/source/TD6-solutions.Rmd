...
\item[{\color{blue}(B)}] Minimizes a convex cost function
\item[(C)] Has a closed-form solution
\item[(D)] None of the above
\end{itemize}
...
\item[{\color{blue}(C)}] Linear relationship between the input variables and the inverse sigmoid of the probability of the event that the outcome $Y = 1$.
...
\begin{itemize}
\item[{\color{blue}(A)}] Same covariance matrix for both classes.
\item[(B)] Same mean for both classes.
\item[(C)] Different covariance matrix for each class.
\item[(D)] None of the above.
\end{itemize}
...
\item[{\color{blue}(C)}] If $\hat{\boldsymbol{\Sigma}}_1 = \hat{\boldsymbol{\Sigma}}_0$, $\hat{\pi}_0 = 1/6$ and $\hat{\pi}_1 = 5/6$, then the LDA and QDA classifiers are identical.
\item[(D)] None of the above.
\end{itemize}
...
\item[{\color{blue}(B)}] 1-nearest-neighbors classifier.
\item[(C)] LDA.
\item[(D)] None of them.
\end{itemize}
...
\end{figure}
\begin{itemize}
\item[{\color{blue}(A)}] 1-NN
\item[(B)] Logistic regression
...
\item[{\color{blue}(D)}] QDA has an advantage over LDA because the two classes are expected to have very different covariance matrices.
\end{itemize}
...
\begin{itemize}
\item[(01)] Decision trees are learned by minimizing information gain {\color{blue}(F)}
\item[(02)] The coefficients $\alpha$ assigned to the classifiers assembled by AdaBoost are always non-negative {\color{blue}(F)}
\item[(03)] Maximizing the likelihood of logistic regression model yields multiple local optima {\color{blue}(F)}
\item[(04)] No classifier can do better than a naive Bayes classifier if the distribution of the data is known {\color{blue}(F)}
\item[(05)] In the discriminative approach to solving classification problems, we model the conditional probability of the labels given the observations {\color{blue}(T)}
\item[(06)] Students from ENSIMAG and PHELMA are trying to solve the same logistic regression problem for a dataset. The PHELMA group claims that their initialization point will lead to a much better optimum than ENSIMAG’s initialization point. PHELMA is correct. {\color{blue}(F)}
\item[(07)] In AdaBoost we start with a Gaussian weight distribution over the training samples {\color{blue}(F)}
\item[(08)] A 1-NN classifier has higher variance than a 3-NN classifier {\color{blue}(T)}
\end{itemize}
